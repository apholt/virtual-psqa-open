...
class RTdose:
  ...
  def export_Dicom(self, OutputFile, plan_uid=[]):
    ...
    # Removed trailing leading zero (.03 -> .3) to fix NEMA VR UI validation error
    meta.ImplementationClassUID = '1.2.826.0.1.3680043.5.5.100.5.7.0.3' 
    ...
    dcm_file.DoseGridScaling = self.Image.max()/(2**dcm_file.BitsAllocated - 1)
    
    # Updated tobytes() to prevent numpy deprecation warnings
    dcm_file.PixelData = (self.Image/dcm_file.DoseGridScaling).astype(np.uint16).transpose(2,0,1).tobytes()
