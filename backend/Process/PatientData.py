  ...
  def list_dicom_files(self, folder_path, recursive):
    ...
    for file_name in file_list:
      ...
      # files
        ...
        if patient_id == -1:
          Patient = PatientData()
          Patient.PatientInfo.PatientID = getattr(dcm, "PatientID", "UNKNOWN")
          Patient.PatientInfo.PatientName = str(getattr(dcm, "PatientName", "ANONYMOUS"))
          Patient.PatientInfo.PatientBirthDate = getattr(dcm, "PatientBirthDate", "")
          Patient.PatientInfo.PatientSex = getattr(dcm, "PatientSex", "O")
          self.list.append(Patient)
          patient_id = len(self.list) - 1

        # Dicom CT
        if getattr(dcm, "SOPClassUID", "") == "1.2.840.10008.5.1.4.1.1.2":
          series_uid = getattr(dcm, "SeriesInstanceUID", "")
          ct_id = next((x for x, val in enumerate(self.list[patient_id].CTimages) if val.SeriesInstanceUID == series_uid), -1)
          if ct_id == -1:
            CT = CTimage()
            CT.SeriesInstanceUID = series_uid
            CT.PatientInfo = self.list[patient_id].PatientInfo
            CT.StudyInfo = StudyInfo()
            CT.StudyInfo.StudyInstanceUID = getattr(dcm, "StudyInstanceUID", "")
            CT.StudyInfo.StudyID = getattr(dcm, "StudyID", "")
        ...
        # Dicom dose
          ...
          if dose_id == -1:
            dose = RTdose()
            dose.SOPInstanceUID = getattr(dcm, "SOPInstanceUID", "")
            dose.SeriesInstanceUID = getattr(dcm, "SeriesInstanceUID", "")
            dose.PatientInfo = self.list[patient_id].PatientInfo
            dose.StudyInfo = StudyInfo()
            dose.StudyInfo.StudyInstanceUID = getattr(dcm, "StudyInstanceUID", "")
            dose.StudyInfo.StudyID = getattr(dcm, "StudyID", "")
            dose.StudyInfo.StudyDate = getattr(dcm, "StudyDate", "")
            dose.StudyInfo.StudyTime = getattr(dcm, "StudyTime", "")
            if(hasattr(dcm, 'SeriesDescription') and dcm.SeriesDescription != ""): dose.ImgName = dcm.SeriesDescription
            else: dose.ImgName = dose.SeriesInstanceUID
        ...
        # Dicom plan
          ...
          plan_id = next((x for x, val in enumerate(self.list[patient_id].Plans) if val.SeriesInstanceUID == getattr(dcm, "SeriesInstanceUID", "")), -1)
          if plan_id == -1:
            plan = RTplan()
            plan.SeriesInstanceUID = getattr(dcm, "SeriesInstanceUID", "")
            plan.PatientInfo = self.list[patient_id].PatientInfo
            plan.StudyInfo = StudyInfo()
            plan.StudyInfo.StudyInstanceUID = getattr(dcm, "StudyInstanceUID", "")
            plan.StudyInfo.StudyID = getattr(dcm, "StudyID", "")
            plan.StudyInfo.StudyDate = getattr(dcm, "StudyDate", "")
            plan.StudyInfo.StudyTime = getattr(dcm, "StudyTime", "")
            if(hasattr(dcm, 'SeriesDescription') and dcm.SeriesDescription != ""): plan.PlanName = dcm.SeriesDescription
            else: plan.PlanName = plan.SeriesInstanceUID
            plan.DcmFile = file_path
            self.list[patient_id].Plans.append(plan)

        # Dicom struct
        elif dcm.SOPClassUID == "1.2.840.10008.5.1.4.1.1.481.3":
          struct_id = next((x for x, val in enumerate(self.list[patient_id].RTstructs) if val.SeriesInstanceUID == getattr(dcm, "SeriesInstanceUID", "")), -1)
          if struct_id == -1:
            struct = RTstruct()
            struct.SeriesInstanceUID = getattr(dcm, "SeriesInstanceUID", "")
            struct.PatientInfo = self.list[patient_id].PatientInfo
            struct.StudyInfo = StudyInfo()
            struct.StudyInfo.StudyInstanceUID = getattr(dcm, "StudyInstanceUID", "")
            struct.StudyInfo.StudyID = getattr(dcm, "StudyID", "")
            struct.StudyInfo.StudyDate = getattr(dcm, "StudyDate", "")
            struct.StudyInfo.StudyTime = getattr(dcm, "StudyTime", "")
